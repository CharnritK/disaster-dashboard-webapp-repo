...
Set `LLM_ENABLED=false` explicitly for deterministic-only demos, local no-key review, and deployments where AI should remain off. Do not treat an unset `LLM_ENABLED` value as an explicit AI opt-out.
...
The browser never reads `LLM_API_KEY`. `NEXT_PUBLIC_COPILOT_API_ENABLED` is the only browser-exposed AI switch in this list. If the recommendation route cannot call an LLM, if `LLM_ENABLED=false`, or if a request exceeds the configured limits, the app falls back to deterministic recommendations. Static Codex Sites builds should keep `NEXT_PUBLIC_COPILOT_API_ENABLED=false` so the browser uses deterministic in-page recommendations instead of calling unavailable API routes.
